...
import { computeCatchUp } from '../utils/catchUpPlan';
...
// v4.113.0: embeddable month calendar extracted from the old MonthHeatmap overlay.
// Lives inside the Goal Tracking view. Props:
//   previewGoalMinutes — live (unsaved) dial selection; null = use saved stats.goalMinutes
//   workDays           — workday basis (17/22/26/28/30 per month); 0 = calendar days
export const MonthCalendarPanel = ({ previewGoalMinutes = null, workDays = 0 }) => {
  const { stats, dailyLog, commitDayToLog, editPastDay, RATE_PER_MINUTE, arsRate } = useSession();
  ...
  // Effective goal: live dial preview while the user is dragging (falls back to saved).
  const effGoal = previewGoalMinutes != null && previewGoalMinutes > 0
    ? previewGoalMinutes
    : stats.goalMinutes;

  // Workday-basis requirement (v4.113.0): same math as every other pace surface.
  const plan = computeCatchUp({
    goalMinutes: effGoal,
    monthlyMinutes: stats.monthlyMinutes,
    dailyMinutes: stats.dailyMinutes,
    workDays,
  });
  const remainingWorkdays = plan.remainingWorkdays;
  const remainingMins     = Math.max(0, effGoal - stats.monthlyMinutes);
  const requiredPerDay    = remainingWorkdays > 0 ? Math.ceil(remainingMins / remainingWorkdays) : 0;

  const isPreviewing = previewGoalMinutes != null && Math.round(previewGoalMinutes) !== Math.round(stats.goalMinutes);
  ...
  // Pace context: how many mins per WORKDAY to hit each tier (v4.113.0 workday basis)
  const paceFor = (tierMins) => {
    const rem = Math.max(0, tierMins - stats.monthlyMinutes);
    return remainingWorkdays > 0 ? Math.ceil(rem / remainingWorkdays) : null;
  };
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <div style={{ fontSize: '0.9rem', fontWeight: 800, color: '#fff' }}>
            ...
            {isPreviewing && <span style={{ fontSize: '0.55rem', color: '#a855f7', marginLeft: '0.4rem' }}>LIVE PREVIEW</span>}
          </div>
          <div style={{ fontSize: '0.6rem', color: '#64748b' }}>
            Day {today}/{daysInMo} · {remainingWorkdays} workdays left · Need {requiredPerDay}m/workday for {isPreviewing ? 'preview goal' : 'current goal'}
          </div>
        </div>
      </div>
            ...
            if (editDay === d) {
              return (
                <div key={d} style={{ borderRadius: '6px', border: '1px solid rgba(239,68,68,0.7)', background: 'rgba(239,68,68,0.15)', padding: '0.15rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.1rem' }}>
                  ...
                  <input
                    ref={inputRef}
                    id={`calendar-day-edit-${d}`}
                    type="number" min="0" max="600"
                    value={editVal}
                    onChange={e => setEditVal(e.target.value)}
                    onKeyDown={e => {
                      if (e.key === 'Enter') commitEdit();
                      if (e.key === 'Escape') { e.stopPropagation(); setEditDay(null); setEditVal(''); }
                    }}
                    onBlur={commitEdit}
      ...
      {/* Legend */}
      <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'center', fontSize: '0.55rem', color: '#475569', flexWrap: 'wrap' }}>
        <span>🟢 On target</span>
        <span>🟡 60–99%</span>
        <span>🔴 &lt;60% / zero</span>
        <span style={{ opacity: 0.5 }}>⬜ Future = need/day</span>
