      ...
      {showBars && (
        <div className="header-metrics-bars">
          <CompactBar
            ...
            onMouseEnter={hover({ icon: <CalendarIcon size={14} />, heading: 'MONTHLY', color: monthlyColor, body: monthlyTooltip })}
            onMouseLeave={leave}
          />
          <CompactBar
            fill={stepFill}
            color={stepColor}
            title={stepTooltip}
            onMouseEnter={hover({ icon: <LadderIcon size={14} />, heading: 'WEEKLY STEP', color: stepColor, body: stepTooltip })}
            onMouseLeave={leave}
          />
          <CompactBar
            fill={dailyFill}
            color={dailyColor}
            title={dailyTooltip}
            onMouseEnter={hover({ icon: <SunIcon size={14} />, heading: 'DAILY', color: dailyColor, body: dailyTooltip })}
            onMouseLeave={leave}
          />
        </div>
      )}
