...
// v4.155.0 / v4.156.0 — the two clinical guards (both OFF by default)
import {
...
export default function SettingsPanel({
  ...
  // v4.155.0 — domain repair switch (OFF by default; see utils/clinicalGuards.js)
...
{section === 'deepgram' && (
          <div style={{ marginTop: 12 }}>
            ...
            {/* v4.155.0 / v4.156.0 — the two clinical guards. OFF = today's behaviour. */}
            <div style={{ marginTop: 14 }}>
              <div style={{ fontSize: 11, color: '#93c5fd', marginBottom: 6 }}>Clinical guards</div>
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                ...
                <button
                  type="button"
                  aria-pressed={negationGuardOn}
                  onClick={() => setNegationGuardOn(setNegationGuardEnabled(!negationGuardOn))}
                  style={{
                    ...tabBtn,
                    background: negationGuardOn ? 'rgba(245, 158, 11, 0.22)' : tabBtn.background,
                  }}
                >
                  Negation guard: {negationGuardOn ? 'ON' : 'OFF'}
                </button>
              </div>
              <p style={{ fontSize: 10, color: 'rgba(255,255,255,0.5)', margin: '6px 0 0' }}>
                <strong>Term repair</strong> fixes known medical/legal words Deepgram hears as plain English —
                “all but a roll” → albuterol, “exit bit” → exhibit. It never touches a number, and it never
